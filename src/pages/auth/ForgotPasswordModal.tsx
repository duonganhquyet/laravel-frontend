...
export const ForgotPasswordModal: React.FC<ForgotPasswordModalProps> = ({ isOpen, onClose }) => {
  ...
  return createPortal(
    <div 
      className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm transition-opacity"
      onClick={(e) => { if (e.target === e.currentTarget) handleClose(); }}
    >
      <div className="bg-white rounded-2xl shadow-xl w-[90%] max-w-md p-6 transform transition-all animate-fade-in-up">
        {/* Header */}
        ...
        {step === 1 ? (
          <form onSubmit={handleSendOtp} className="space-y-5 text-left">
            <p className="text-sm text-slate-500 font-medium">
              Vui lòng nhập địa chỉ email bạn đã dùng để đăng ký tài khoản. Chúng tôi sẽ gửi cho bạn một mã OTP để xác thực.
            </p>
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoading && (
          ...
          <form onSubmit={handleResetPassword} className="space-y-5 text-left">
            <p className="text-sm text-slate-500 font-medium">
              Vui lòng nhập mã OTP đã được gửi đến <strong>{email}</strong> và đặt lại mật khẩu mới.
            </p>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all text-slate-800 text-center tracking-widest font-mono text-lg"
                required
              />
            </div>
            ...
            <div className="space-y-1.5 relative">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all text-slate-800 pr-12"
                  required
                />
                <button
                  type="button"
            ...
            <div className="space-y-1.5 relative">
              <label className="text-sm font-semibold text-slate-700 ml-1">Xác nhận mật khẩu mới</label>
              <div className="relative">
                <input
                  type={showPassword ? "text" : "password"}
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded-xl transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-emerald-500/20"
              >
                {isLoading && (
                  <svg className="animate-spin w-4 h-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
